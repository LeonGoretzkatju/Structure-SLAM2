...
int main(int argc, char **argv) {
    ...
    std::vector<rs2::sensor> sensors = selected_device.query_sensors(); //get the sensors(camera imu .etc) of the current device.
    int index = 0;
    ...
    while (b_continue_session)
    {
        ...
        {
            ORB_SLAM2::IMU::Point lastPoint(vAccel[i].x, vAccel[i].y, vAccel[i].z,
        ...
        std::cout << "enter before trackrgbd" << std::endl;
