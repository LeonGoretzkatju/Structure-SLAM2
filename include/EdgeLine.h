...
class EdgeInertialLineProjectXYZ
        : public g2o::BaseBinaryEdge<3, Eigen::Vector3d, g2o::VertexSBAPointXYZ, ORB_SLAM2::VertexPose> {
    typedef ORB_SLAM2::VertexPose VertexPose;
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW

    EdgeInertialLineProjectXYZ(int cam_idx_=0):cam_idx(cam_idx_){
    }
    ...
    virtual void computeError() {
        ...
        const Eigen::Vector2d proj = v1->estimate().Project(v2->estimate(),cam_idx);
        _error(0) = obs(0) * proj(0) + obs(1) * proj(1) + obs(2);
        _error(1) = 0;
        _error(2) = 0;
    }
    ...
    virtual void linearizeOplus() {
        ...
        // const Eigen::Matrix3d &Rcw = vj->estimate().Rcw[cam_idx];
        // const Eigen::Vector3d &tcw = vj->estimate().tcw[cam_idx];

        // Eigen::Vector3d xyz_trans = Rcw*vi->estimate()+tcw;
        ...
        // const double& lx = _measurement(0);
        // const double& ly = _measurement(1);
        ...
        // Eigen::Matrix<double, 3, 3, Eigen::ColMajor> dProject;
        // dProject = Eigen::Matrix3d::Zero();
        // dProject(0, 0) = fx * lx;
        // dProject(0, 1) = fy * ly;
        // dProject(0, 2) = -(fx * lx * x + fy * ly * y) * invz;

        // _jacobianOplusXi = 1. * invz * dProject * Rcw;
    }
...
class EdgeInertialLineProjectXYZOnlyPose : public g2o::BaseUnaryEdge<3, Eigen::Vector3d, ORB_SLAM2::VertexPose> {
    typedef ORB_SLAM2::VertexPose VertexPose;
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW;

    EdgeInertialLineProjectXYZOnlyPose(const Eigen::Vector3d &Xw_, int cam_idx_=0):Xw(Xw_),
            cam_idx(cam_idx_){}

    virtual void computeError() {
        const VertexPose* VPose = static_cast<const VertexPose*>(_vertices[0]);
        Eigen::Vector3d obs = _measurement;
        Eigen::Vector2d proj = VPose->estimate().Project(Xw,cam_idx);
        _error(0) = obs(0) * proj(0) + obs(1) * proj(1) + obs(2);
        _error(1) = 0;
        _error(2) = 0;
    }
    ...
    virtual void linearizeOplus() {
       BaseUnaryEdge::linearizeOplus();
        // const VertexPose* VPose = static_cast<const VertexPose*>(_vertices[0]);
        // Eigen::Vector3d xyz_trans = VPose->estimate().map(Xw);
