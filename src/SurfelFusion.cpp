...
void SurfelFusion::fuse_initialize_map(
    ...
    std::vector<SurfelElement> &new_surfels)
{
    ...
    pose = cam_pose;
    planeMembershipImg = input_planeMembershipImg;
    ...
    printf("SurfelFusion::generate superpixels cost %f ms.\n", total_time.count() * 1000.0);
    start_time = std::chrono::system_clock::now();
    ...
    for (int i = 0; i < THREAD_NUM; i++)
    {
        std::thread this_thread(
            &SurfelFusion::fuse_surfels_kernel, this, i, THREAD_NUM,
    ...
    end_time = std::chrono::system_clock::now();
    total_time = end_time - start_time;
    printf("SurfelFusion::fuse superpixels cost %f ms.\n", total_time.count() * 1000.0);
    start_time = std::chrono::system_clock::now();
    ...
    end_time = std::chrono::system_clock::now();
    total_time = end_time - start_time;
    printf("SurfelFusion::initialize superpixels cost %f ms.\n", total_time.count() * 1000.0);
}
...
void SurfelFusion::back_project(
...
float SurfelFusion::get_weight(float &depth)
{
    return std::min(1.0 / depth / depth, 1.0);
}

void SurfelFusion::get_huber_norm(
...
void SurfelFusion::fuse_surfels_kernel(
    ...
    int reference_frame_index)
{
    ...
    for (int i = begin_index; i < end_index; i++)
    {
        ...
        local_surfels[i].r = superpixel_seeds[sp_index].r;
        local_surfels[i].g = superpixel_seeds[sp_index].g;
        local_surfels[i].b = superpixel_seeds[sp_index].b;
...
void SurfelFusion::initialize_surfels(
    int reference_frame_index,
    const Eigen::Matrix4f& pose)
{
    ...
    for (int i = 0; i < superpixel_seeds.size(); i++)
    {
        ...
        new_ele.r = superpixel_seeds[i].r;
        new_ele.g = superpixel_seeds[i].g;
        new_ele.b = superpixel_seeds[i].b;
...
// superpixel related
bool SurfelFusion::calculate_cost(
...
void SurfelFusion::update_pixels_kernel(
    int thread_i, int thread_num)
{
    ...
    {
        if (planeMembershipImg.at<int>(row_i / 2, col_i / 2) != -1) {
            continue;
        }
...
void SurfelFusion::update_pixels()
{
    std::vector<std::thread> thread_pool;
    for (int i = 0; i < THREAD_NUM; i++)
    {
        std::thread this_thread(&SurfelFusion::update_pixels_kernel, this, i, THREAD_NUM);
        thread_pool.push_back(std::move(this_thread));
    }
...
void SurfelFusion::update_seeds_kernel(
    int thread_i, int thread_num)
{
    ...
    {
        if(!superpixel_seeds[seed_i].use)
            continue;
        ...
        cv::Vec3b rgb = image.at<cv::Vec3b>(sum_y,sum_x);
        superpixel_seeds[seed_i].r = rgb[0];
        superpixel_seeds[seed_i].g = rgb[1];
        superpixel_seeds[seed_i].b = rgb[2];
...
void SurfelFusion::update_seeds()
{
    std::vector<std::thread> thread_pool;
    for (int i = 0; i < THREAD_NUM; i++)
    {
        std::thread this_thread(&SurfelFusion::update_seeds_kernel, this, i, THREAD_NUM);
        thread_pool.push_back(std::move(this_thread));
    }
    for (int i = 0; i < thread_pool.size(); i++)
        if (thread_pool[i].joinable())
...
void SurfelFusion::initialize_seeds_kernel(
    int thread_i, int thread_num)
{
    int step = superpixel_seeds.size() / thread_num;
    int begin_index = step * thread_i;
    ...
    for (int seed_i = begin_index; seed_i < end_index; seed_i++)
    {
        ...
        if (planeMembershipImg.at<int>(image_y / 2, image_x / 2) != -1) {
            superpixel_seeds[seed_i].use = false;
            continue;
        }

        SuperpixelSeed this_sp;
        this_sp.x = image_x;
        this_sp.y = image_y;
        cv::Vec3b rgb = image.at<cv::Vec3b>(image_y,image_x);
        this_sp.r = rgb[0];
        this_sp.g = rgb[1];
        this_sp.b = rgb[2];
...
void SurfelFusion::initialize_seeds()
{
    std::vector<std::thread> thread_pool;
    for (int i = 0; i < THREAD_NUM; i++)
    {
        std::thread this_thread(&SurfelFusion::initialize_seeds_kernel, this, i, THREAD_NUM);
        thread_pool.push_back(std::move(this_thread));
    }
    for (int i = 0; i < thread_pool.size(); i++)
        if (thread_pool[i].joinable())
...
void SurfelFusion::calculate_spaces_kernel(int thread_i, int thread_num)
{
    int step_row = image_height / thread_num;
    int start_row = step_row * thread_i;
    int end_row = start_row + step_row;
...
void SurfelFusion::calculate_pixels_norms_kernel(int thread_i, int thread_num)
{
    int step_row = image_height / thread_num;
    int start_row = step_row * thread_i;
...
void SurfelFusion::calculate_sp_norms_kernel(int thread_i, int thread_num)
{
    int step = superpixel_seeds.size() / thread_num;
    int begin_index = step * thread_i;
    int end_index = begin_index + step;
...
void SurfelFusion::calculate_sp_depth_norms_kernel(int thread_i, int thread_num)
{
    int step = superpixel_seeds.size() / thread_num;
    int begin_index = step * thread_i;
    int end_index = begin_index + step;
...
void SurfelFusion::calculate_norms()
{
    std::vector<std::thread> thread_pool;
    for (int i = 0; i < THREAD_NUM; i++)
    {
        std::thread this_thread(&SurfelFusion::calculate_spaces_kernel, this, i, THREAD_NUM);
        thread_pool.push_back(std::move(this_thread));
    }
    for (int i = 0; i < thread_pool.size(); i++)
        if (thread_pool[i].joinable())
    ...
    for (int i = 0; i < THREAD_NUM; i++)
    {
        std::thread this_thread(&SurfelFusion::calculate_pixels_norms_kernel, this, i, THREAD_NUM);
        thread_pool.push_back(std::move(this_thread));
    }
    for (int i = 0; i < thread_pool.size(); i++)
        if (thread_pool[i].joinable())
    ...
    // for (int i = 0; i < THREAD_NUM; i++)
    // {
    //     std::thread this_thread(&SurfelFusion::calculate_sp_norms_kernel, this, i, THREAD_NUM);
    //     thread_pool.push_back(std::move(this_thread));
    // }
    // for (int i = 0; i < thread_pool.size(); i++)
    //     if (thread_pool[i].joinable())
    ...
    for (int i = 0; i < THREAD_NUM; i++)
    {
        std::thread this_thread(&SurfelFusion::calculate_sp_depth_norms_kernel, this, i, THREAD_NUM);
        thread_pool.push_back(std::move(this_thread));
    }
    for (int i = 0; i < thread_pool.size(); i++)
        if (thread_pool[i].joinable())
...
void SurfelFusion::generate_super_pixels() {
    // std::fill(superpixel_seeds.begin(), superpixel_seeds.end(), 0);
    memset(superpixel_seeds.data(), 0, superpixel_seeds.size() * sizeof(SuperpixelSeed));
...
//        for (int col_i = 0; col_i < image_width; col_i++) {
//            int plid = planeMembershipImg.at<int>(row_i / 2, col_i / 2);
//            if (plid < 0) {
//                continue;
//            }
//
//            int index = row_i * image_width + col_i;
////            std::cout << "plid:" << plid << std::endl;
////            std::cout << "Index:" << index/8 << std::endl;
////            std::cout << "superpixel_seeds:" << superpixel_seeds.size() << std::endl;
//
//            if (map.count(plid)) {
//                superpixel_index[index] = map[plid];
//            } else {
//                SuperpixelSeed this_sp;
//                this_sp.x = col_i;
//                this_sp.y = row_i;
//                this_sp.mean_intensity = image.at<uchar>(row_i, col_i);
//                this_sp.fused = false;
//                this_sp.stable = false;
//                this_sp.mean_depth = depth.at<float>(row_i, col_i);
//                map[plid] = superpixel_seeds.size();
//                superpixel_seeds.push_back(this_sp);
//            }
//        }
//    }
...
void SurfelFusion::debug_show()
{
    const unsigned char default_colors[15][3] =
            {
                    {255, 0, 0},
                    {255, 255, 0},
                    {100, 20, 50},
    ...
    for (int j = 0; j < image_height; j++)
        ...
        {
            int plid = planeMembershipImg.at<int>(j / 2, i / 2);
            if (plid != -1) {
                result.at<cv::Vec3b>(j, i) = cv::Vec3b(default_colors[plid][0], default_colors[plid][1], default_colors[plid][2]);
                continue;
            }
            // pixel sp index
