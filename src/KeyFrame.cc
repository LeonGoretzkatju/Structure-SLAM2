...
namespace ORB_SLAM2
{
    ...
    KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB):
            ...
            mpCamera(F.mpCamera), mpCamera2(F.mpCamera2), bImu(pMap->isImuInitialized()), mpImuPreintegrated(F.mpImuPreintegrated),
            mImuCalib(F.mImuCalib), mPrevKF(NULL), mNextKF(NULL)
    {
        mnId=nNextId++;
    ...
    void KeyFrame::SetPose(const cv::Mat &Tcw_)
    {
        ...
        if (!mImuCalib.Tcb.empty())
            Owb = Rwc*mImuCalib.Tcb.rowRange(0,3).col(3)+Ow; // IMU
...
//        }
    ...
    Vector6d KeyFrame::obtain3DLine(const int &i) {
        double len = cv::norm(mvKeyLines[i].getStartPoint() - mvKeyLines[i].getEndPoint());
...
//        }
    ...
    cv::Mat KeyFrame::ComputePlaneWorldCoeff(const int &idx) {
        return Tcw.t()*mvPlaneCoefficients[idx];
    }
    ...
    void KeyFrame::AddMapVerticalPlane(ORB_SLAM2::MapPlane *pMP, const int &idx) {
        unique_lock<mutex> lock(mMutexFeatures);
        mvpVerticalPlanes[idx] = pMP;
    }

    void KeyFrame::AddMapParallelPlane(ORB_SLAM2::MapPlane *pMP, const int &idx) {
        unique_lock<mutex> lock(mMutexFeatures);
